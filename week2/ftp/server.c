...
int main()
{
	...
	if (bind(server_sock,(struct sockaddr*)&my_addr,(socklen_t)sizeof(my_addr)) < 0)
	...
	while(1){
		...
		if( (accept_sockfd = accept(server_sock, (struct sockaddr*)&their_addr, (socklen_t*)&addr_len)) < 0)
			error("accept");
        printf("Connection received\n");
		
        char welcome_message[] = "220 Welcome343434434 to MyFTP Server\r\n";
        send(accept_sockfd, welcome_message, strlen(welcome_message), 0);
